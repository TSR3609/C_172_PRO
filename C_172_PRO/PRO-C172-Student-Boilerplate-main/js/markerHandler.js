...
AFRAME.registerComponent("markerhandler", {
      ...
      */
      ...
      orderButtton.addEventListener("click", () => {
        ...
        swal({
          ...
          text: "The toy will be delivered soon",
          timer: 2000,
          buttons: false
        });
      });
  ...
  handlePayment: function () {
    ...
    var tablenumber_
    tablenumber_ <=9?(tablenumber_ = `t0${tablenumber_}`):(`t${tablenumber_}`)
    firebase.firestore().collection("tables").doc(tablenumber_).update({current_orders:{},total_bill:0})
    .then(()=>{
      swal({
        icon:"success",
